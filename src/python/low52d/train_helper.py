...
def train(opt):
	...
	dt = pd.read_csv('downloads/NSE-' + opt
	                 + '.csv')
	dt_length = len(dt)
	...
	logging.info('Processing Stock:%s',opt)
	 # converting to array
	...
	for x in margin:
		...
		if run_result[0]:
			logging.info('%s:::margin:%s==>%s',opt,x,run_result[1])
			res_ls.append(run_result[1])
			ror_ls.append(run_result[1]['ror'])
		else:
			logging.info('%s:::margin:%s==>no transaction',opt,x,)
	result={}
	if len(ror_ls)>0:
...
def evaluator_config(ror_dt):
	...
	y=0
	for x in nrr_ls:
		nrr.append({'strategy_id':'LOW52D','tradingsymbol':stk_ls[y],'nrr':x,'profit_margin':margin_ls[y],"buy_price_threshold": buy_price_threshold,'stock_ceil':stock_ceil})
		y=y+1
	return nrr
